...
Factual constraints:
- Use details directly visible in the provided key frames as well as semantic information from ASR audio transcripts; do not fill in off-screen or unspoken content or speculate about future developments.
- When audio transcripts are available, explicitly mentioned entity names, numbers, and event context take priority over purely visual inference.
...
事实性约束：
- 使用关键帧中直接可见的细节以及 ASR 音频转录中的语义信息；不要补全画面外或音频未提及的内容或推测后续发展。
- 当音频转录可用时，音频中明确提到的实体名称、数字、事件背景的优先级高于纯视觉推断。
...
class SummarizerModel:
    def __init__(
        ...
        asr_base = asr_api_base or api_base
        if asr_base != api_base or asr_model_name != model_name:
            self._asr_client = OpenAI(
                api_key=api_key,
                base_url=asr_base,
            )
        else:
            self._asr_client = self._client
        ...
        self._tokenizer_failed = self._tokenizer_cls is None
    ...
    def _chat(self, messages: list, max_tokens: int, temperature: float = 0.3, top_p: float = 0.9,
        ...
        # [VLLM] extra_body = {"greedy": False}
        # [VLLM] if top_k > 0:
        # [VLLM]     extra_body["top_k"] = top_k
        # [VLLM] if repetition_penalty != 1.0:
        # [VLLM]     extra_body["repetition_penalty"] = repetition_penalty
    ...
    def transcribe_audio(self, audio_base64: str) -> str:
        ...
        Sends audio as input_audio content type.
        Returns the transcribed text, or empty string on failure.
        """
        try:
            data_url = f"data:audio/wav;base64,{audio_base64}"
            response = self._asr_client.chat.completions.create(
            ...
            print(f"WARNING: ASR transcription failed: {e}")
            return ""
    ...
    @staticmethod
    def _build_audio_section(audio_transcripts: list) -> str:
        """Build the audio transcript section for the mid-term summary prompt."""
        if not audio_transcripts:
            return ""
        lines = [
            "【音频转录（ASR）—— 与视觉帧同等重要的信息源】",
            "以下音频转录文本是你撰写记忆时必须深度整合的核心信息，不是可选参考。",
            "",
            "[融合规则]",
            "- 实体命名：用 ASR 中提到的具体人名、地名、术语、品牌来命名视觉帧中看到的人和物，",
            "  同一实体在视觉和音频中的称呼不一致时，以音频中的明确称呼为准，视觉描述作为外观补充。",
            "- 事件理解：用音频中的对话、指令、评论来解释视觉帧中动作的目的、背景和因果，",
            "  例如视觉看到「一个人击球」，音频听到「注意手腕动作，弧度要大」→ 记忆写「教练指导学员注意手腕动作和弧线」。",
            "- 数字与数据：ASR 中提到的数字（比分、时长、数量等）应直接写入记忆，并与视觉核对一致性。",
            "- 冲突处理：当音频与视觉矛盾时，优先采信音频的语义内容，视觉作为场景验证；",
            "  若无法判定，保留两种表述并标注来源（如「音频中提到 X，但画面显示 Y」）。",
            "",
            "[转录文本]",
        ]
        for entry in audio_transcripts:
            time_range = entry.get("time_range", "unknown")
    ...
    def generate_detailed_summary(
        ...
        audio_section = self._build_audio_section(audio_transcripts or [])
    ...
    def batch_compress_to_longterm(
        ...
        # Build existing long-term memory section as reference context
        existing_longterm_section = ""
        if existing_longterm:
            existing_longterm_section = (
                "已有的长期记忆（仅供参考上下文，避免重复已压缩的信息）：\n"
                + existing_longterm
            )
        ...
        prompt_text = BATCH_COMPRESS_PROMPT.format(
            ...
            length_instruction=length_instruction,
            merged_range=merged_range,
            existing_longterm_section=existing_longterm_section,
        )
        debug_input = None
        if self.debug:
    ...
    def shutdown(self):
        """No-op: the API server is managed externally."""
        pass
