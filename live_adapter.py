...
class AdapterConfig:
    ...
    main_api_base: str = "http://localhost:8000/v1"
    main_model: str = "qwen3-omni-flash"
    ...
    summarizer_api_base: str = "http://localhost:8000/v1"
    longterm_model: str = "qwen3-omni-flash"
    longterm_api_base: str = "http://localhost:8000/v1"
...
class StreamingInferAdapter:
    ...
    async def _handle_chat_payload(
        ...
        *,
        ...
        # ASR: extract audio, transcribe, accumulate to chunk
        audio_base64 = _extract_audio_base64(messages)
        audio_transcript_text = ""
        if audio_base64 and self.summarizer is not None:
            try:
                audio_transcript_text = await asyncio.to_thread(
                    self.summarizer.transcribe_audio, audio_base64
                )
            except Exception as exc:
                LOGGER.warning("[%s] ASR transcription error: %s", state.session_id, exc)
        if audio_transcript_text:
            state.current_chunk["audio_transcripts"].append({
                "time_range": time_ranges[0] if time_ranges else "unknown",
                "text": audio_transcript_text,
            })
            if self._async_summary_enabled():
                state.async_summary_segment["audio_transcripts"].append({
                    "time_range": time_ranges[0] if time_ranges else "unknown",
                    "text": audio_transcript_text,
                })
    ...
    def _main_generation_kwargs(self, inbound_payload: dict[str, Any]) -> dict[str, Any]:
        ...
        # [VLLM] extra_body.setdefault("skip_special_tokens", False)
        # [VLLM] extra_body.setdefault("greedy", False)
        if self.config.honor_inbound_generation_params:
            # [VLLM] extra_body.setdefault("top_k", self.config.main_top_k)
            # [VLLM] extra_body.setdefault("repetition_penalty", self.config.main_repetition_penalty)
            return {
        ...
        # [VLLM] extra_body["top_k"] = self.config.main_top_k
        # [VLLM] extra_body["repetition_penalty"] = self.config.main_repetition_penalty
        return {
    ...
    def _build_mid_term_summary_entry(
        ...
        summary, mid_term_debug_input = self.summarizer.generate_detailed_summary(
            ...
            resolved_query_text,
            audio_transcripts=chunk_snapshot.get("audio_transcripts", []),
        )
...
def _extract_audio_base64(messages: list[dict[str, Any]]) -> Optional[str]:
    """Extract the first base64 audio data from the last user message."""
    for message in reversed(messages):
        if message.get("role") != "user":
            continue
        content = message.get("content")
...
def parse_args() -> AdapterConfig:
    ...
    parser.add_argument(
        "--main-model",
        default=os.environ.get("MAIN_MODEL", "qwen3-omni-flash"),
    )
    parser.add_argument(
        "--main-backends",
    ...
    parser.add_argument(
        ...
        default=os.environ.get(
            ...
            os.environ.get("SUMMARIZER_MODEL", "qwen3-omni-flash"),
        ),
    )
    parser.add_argument(
        "--longterm-api-base",
